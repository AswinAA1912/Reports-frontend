...
import { PendingSaleOrderReport, PendingSaleOrderItemReport } from "../../services/saleOrderReport.service";
...
const PendingSaleOrder: React.FC = () => {
    ...
    const reportTitleName = showCancelledOnly ? "Cancelled Sale Order" : "Pending Sale Order";
    ...
    useEffect(() => {
        const service =
            toggleMode === "Expanded"
                ? PendingSaleOrderItemReport.getPendingSaleOrderItem
                : PendingSaleOrderReport.getPendingSaleOrder;
        ...
        service({
            ...
            .catch(err => {
                console.error("Pending Sale Order Report API Error:", err);
            })
            .finally(() => {
                setLoading(false);
            });
    ...
    }, [rawRows, filters, rangeFilter, showCancelledOnly]);
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(
            wb,
            `${reportTitleName} Report_${toggleMode}_${dayjs().format("DDMMYYYY")}.xlsx`
        );
    };
    ...
    const handleExportPDF = () => {
        ...
        doc.text(`${reportTitleName} Report (${toggleMode})`, 14, 10);
        ...
        doc.save(
            `${reportTitleName} Report_${toggleMode}_${dayjs().format("DDMMYYYY")}.pdf`
        );
    };
    ...
    return (
        <>
            ...
            <ReportFilterDrawer
                ...
                showSaleOrderStatusFilter={true}
                saleOrderStatusValue={showCancelledOnly ? "cancelled" : "pending"}
                onSaleOrderStatusChange={(value) => {
                    setShowCancelledOnly(value === "cancelled");
                    setPage(1);
                }}
            />
...
export default PendingSaleOrder;